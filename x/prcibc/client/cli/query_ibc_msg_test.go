...
func TestShowIbcMsg(t *testing.T) {
	net, objs := networkWithIbcMsgObjects(t, 2)
	...
	for _, tc := range []struct {
		...
		t.Run(tc.desc, func(t *testing.T) {
			...
			out, err := clitestutil.ExecTestCLICmd(ctx, cli.CmdShowIbcMsg(), args)
			if tc.err != nil {
				...
				require.NoError(t, err)
				var resp types.QueryGetIbcMsgResponse
...
func TestListIbcMsg(t *testing.T) {
	net, objs := networkWithIbcMsgObjects(t, 5)
	...
	t.Run("ByOffset", func(t *testing.T) {
		...
		for i := 0; i < len(objs); i += step {
			...
			out, err := clitestutil.ExecTestCLICmd(ctx, cli.CmdListIbcMsg(), args)
			require.NoError(t, err)
			var resp types.QueryAllIbcMsgResponse
			require.NoError(t, net.Config.Codec.UnmarshalJSON(out.Bytes(), &resp))
